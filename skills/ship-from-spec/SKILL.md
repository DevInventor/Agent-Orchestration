...
# /ship-from-spec â€” ship an existing spec
...
and stop â€” do not invent a spec.
...
2. **Name the workstream, then create its bus.** The slug is derived from the doc's
   path by `pipe.py`, never by eye â€” a later wave must derive the same one, or it lands
   on a second branch.
   ```bash
...
2c. **Start the dashboard â€” do not skip this.** Nothing else in the pipeline ever starts
...
3. Write the spec into `<bus>/spec.md` (overwrite). Preserve the user's content;
   only add structure if it lacks crisp, testable acceptance criteria â€” in that case
